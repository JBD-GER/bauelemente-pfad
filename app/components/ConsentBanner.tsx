...
import { readConsent, writeConsent, updateGcmV2 } from "../lib/consent/client";
...
export default function ConsentBanner() {
  ...
  // Open via global event (Footer-Link etc.)
  useEffect(() => {
    const onOpen = () => {
    ...
    window.addEventListener("bp:open-consent", onOpen);
    return () => window.removeEventListener("bp:open-consent", onOpen);
  }, []);
  ...
  // ESC schließt (nur wenn Consent existiert ODER REQUIRE_CHOICE=false)
  useEffect(() => {
    if (!open) return;
    const onKey = (e: KeyboardEvent) => e.key === "Escape" && close("esc");
  ...
  const summary = useMemo(() => {
    const parts: string[] = [t.necessary];
    if (analytics) parts.push(t.analytics);
    if (marketing) parts.push(t.marketing);
    return parts.join(" · ");
  }, [analytics, marketing, t]);

  function close(source: "backdrop" | "esc" | "button" = "button") {
    const current = readConsent();

    if (!current) {
      if (REQUIRE_CHOICE) return;
      // Soft: wegklicken = ablehnen
  ...
  return (
    <div
      className={cn("fixed inset-0 z-[80]", "flex items-end justify-center p-4 sm:p-6")}
      role="dialog"
      aria-modal="true"
      aria-label={t.aria}
    >
      ...
      <div className="relative w-full max-w-[920px]">
        <div
          className={cn(
            "relative overflow-hidden rounded-[1.6rem] border border-white/60 bg-white/70",
            "shadow-[0_20px_70px_rgba(15,23,42,0.14)] backdrop-blur-xl"
          )}
        >
          <div className="pointer-events-none absolute inset-0 opacity-55">
            <div className="bp-consent-sheen absolute -left-1/2 top-0 h-full w-[200%]" />
          </div>

          <div className="relative p-4 sm:p-5">
            <div className="inline-flex items-center gap-2 rounded-full border border-white/60 bg-white/70 px-3 py-1 text-[11px] font-medium text-slate-700 shadow-sm backdrop-blur">
              <span className="inline-block h-1.5 w-1.5 rounded-full" style={{ backgroundColor: PRIMARY, opacity: 0.55 }} />
              {t.badge}
            </div>

            <div className="mt-3">
              <div className="text-[18px] font-semibold tracking-tight text-slate-900 sm:text-[20px]">{t.title}</div>

              <p className="mt-2 max-w-[980px] text-[12px] leading-relaxed text-slate-700 sm:text-[13px]">{t.text}</p>
      ...
      <style>{`
        .bp-consent-sheen{
          background: linear-gradient(
            90deg,
            transparent 0%,
            rgba(7,24,61,0.05) 35%,
            rgba(7,24,61,0.10) 50%,
            rgba(7,24,61,0.05) 65%,
            transparent 100%
          );
        ...
        @media (prefers-reduced-motion: reduce){
          .bp-consent-sheen{ animation:none !important; transform:none !important; }
        }
      `}</style>
    </div>
  );
